...
if ((!isset($logeado) && !isset($TxtUsuario))){
	...
	// 	// ------------------------------------------------------------------- BANNER ENCABEZADO --------------------------------------------------------------- 
	// 	$TItulo="<h3 class='fbold ftext-red fcenter'>Sector Escolar ".$_SESSION["vg_cct_zona"]."</h3>"; $subtit=""; 		 
	...
	// }else{ 
       ...
       //     $elidnivel=$_GET["idnivel"];
		...
		<div style='margin-left:50px; margin-right:50px;  font-weight:bold'>
                    ...
                    <br><br>   <?php    
//---------------------------------------------------------------------------------------------------------------------------------------------- 
                    $_sql="select a.cuenta,a.nombre,a.sector,a.nommun,a.folio,b.almadis,sum(b.alu_1) as alu1,sum(b.alu_2) as alu2,sum(b.alu_3) as alu3,sum(b.alu_4) as alu4,sum(b.alu_5) as alu5,sum(b.alu_6) as alu6 from sector_c a,ctsev b where a.cuenta=b.cct_sector and b.estatus='A' and b.cct_sector='".$elsector."' group by b.cct_sector"; //	echo $_sql; 
                    ...
                    <table class="ftable-print-horizontal" cellpadding="5" cellspacing="0" width="100%" border=1 style="font-weight:bold">
                        <tr><td width="10%">Folio</td><td  width="20%">LTG <? echo $rec_sector_alumno["folio"]; ?></td><td width="14%">Almacen</td><td><? echo  $_SESSION["logeado"]; ?></td></tr>
                    ...
                    if (mysql_num_rows($res_libros)>0){  ?>
                        ...
                        <table class="ftable-print" cellpadding="5" cellspacing="0" width="100%" border=1>
                            ...
                            while ($r<mysql_num_rows($res_libros)){ 		$reclibros=mysql_fetch_array($res_libros,MYSQL_ASSOC); 
                                $tot_cajas=$tot_cajas+($rec_sector_alumno["alu1"]/$reclibros["primcajas"]);         $tot_sueltos=$tot_sueltos+($rec_sector_alumno["alu1"]-($reclibros["primcajas"]*intval($rec_sector_alumno["alu1"]/$reclibros["primcajas"])));?>
                                        <tr><td><? echo $reclibros["clave"]; ?></td><td><? echo $reclibros["titulo"]; ?></td><td style="text-align:right;"><? echo number_format($rec_sector_alumno["alu1"],0); ?></td><td style="text-align:right;"><? echo number_format($reclibros["primcajas"],0); ?></td><td style="text-align:right;"><? echo number_format(floor($rec_sector_alumno["alu1"]/$reclibros["primcajas"]),0); ?></td><td style="text-align:right;"><? echo number_format($rec_sector_alumno["alu1"]-($reclibros["primcajas"]*intval($rec_sector_alumno["alu1"]/$reclibros["primcajas"])),0); ?></td></tr><?
                                $r++;        
                            }?>                            
                    ...
                    if (mysql_num_rows($res_libros)>0){  ?>
                        ...
                        <div style='font-size:18px; font-weight:bold; margin:10px'>Segundo Grado</div>
                            ...
                            $tot_cajas=0;   $tot_sueltos=0; 
                            while ($r<mysql_num_rows($res_libros)){ 		$reclibros=mysql_fetch_array($res_libros,MYSQL_ASSOC); 
                                $tot_cajas=$tot_cajas+($rec_sector_alumno["alu2"]/$reclibros["primcajas"]);         $tot_sueltos=$tot_sueltos+($rec_sector_alumno["alu2"]-($reclibros["primcajas"]*intval($rec_sector_alumno["alu2"]/$reclibros["primcajas"])));?>
                                        <tr><td><? echo $reclibros["clave"]; ?></td><td><? echo $reclibros["titulo"]; ?></td><td style="text-align:right;"><? echo number_format($rec_sector_alumno["alu2"],0); ?></td><td style="text-align:right;"><? echo number_format($reclibros["primcajas"],0); ?></td><td style="text-align:right;"><? echo number_format(floor($rec_sector_alumno["alu2"]/$reclibros["primcajas"]),0); ?></td><td style="text-align:right;"><? echo number_format($rec_sector_alumno["alu2"]-($reclibros["primcajas"]*intval($rec_sector_alumno["alu2"]/$reclibros["primcajas"])),0); ?></td></tr><?
                                $r++;        
                            }?>                            
                            <tr><td colspan="4" style="text-align:right; font-weight:bold">Total&emsp;<td style="text-align:right; font-weight:bold"><? echo number_format($tot_cajas,0); ?></td><td style="text-align:right; font-weight:bold"><? echo number_format($tot_sueltos,0); ?></td></tr>
                        </table><br><br>                       <?      
                    ...
                    if (mysql_num_rows($res_libros)>0){  ?>
                        ...
                        <table class="ftable-print" cellpadding="5" cellspacing="0" width="100%" border=1>
                            ...
                            $tot_cajas=0;   $tot_sueltos=0; 
                            while ($r<mysql_num_rows($res_libros)){ 		$reclibros=mysql_fetch_array($res_libros,MYSQL_ASSOC); 
                                $tot_cajas=$tot_cajas+($rec_sector_alumno["alu3"]/$reclibros["primcajas"]);         $tot_sueltos=$tot_sueltos+($rec_sector_alumno["alu3"]-($reclibros["primcajas"]*intval($rec_sector_alumno["alu3"]/$reclibros["primcajas"])));?>
                                        <tr><td><? echo $reclibros["clave"]; ?></td><td><? echo $reclibros["titulo"]; ?></td><td style="text-align:right;"><? echo number_format($rec_sector_alumno["alu3"],0); ?></td><td style="text-align:right;"><? echo number_format($reclibros["primcajas"],0); ?></td><td style="text-align:right;"><? echo number_format(floor($rec_sector_alumno["alu3"]/$reclibros["primcajas"]),0); ?></td><td style="text-align:right;"><? echo number_format($rec_sector_alumno["alu3"]-($reclibros["primcajas"]*intval($rec_sector_alumno["alu3"]/$reclibros["primcajas"])),0); ?></td></tr><?
                                $r++;        
                            }?>                            
                            <tr><td colspan="4" style="text-align:right; font-weight:bold">Total&emsp;<td style="text-align:right; font-weight:bold"><? echo number_format($tot_cajas,0); ?></td><td style="text-align:right; font-weight:bold"><? echo number_format($tot_sueltos,0); ?></td></tr>
                        </table><br><br>                       <?      
...
//---------------------------------------------------------------------------------------------------------------------------------------------- ?>
                    ...
                    <table cellpadding="10" cellspacing="0" width="100%" border=1>
                        <tr><td width="15%" align="center"><? echo GenerarQR("LTG" . number_format( $rec_sector_alumno["folio"],0), 'qr_sector_preescolar/'); ?> </td><td style="font-size:18px; font-weight:bold;">Observaciones</td></tr>
