    <?php	
function flista_niveles_x_almacen($pmalmacen,$nivel_sinpermiso="",$liga1=""){    
   ...
   // echo "flista_niveles_x_almacen<br>".$_sql;
    $res=mysql_query($_sql);    $vl_regs=mysql_num_rows($res);        $vl_pontotales=$vl_regs;				
    if ($vl_regs>0){ ?>		
        <table class="ftable-all"><tr>
            <th>Nivel</th><th>Sectores</th> <th></th><th>Zonas</th><th></th><? $r=0;
            while ($r<$vl_regs){
                ...
                if ($recset["ban"]>0){  
                    ...
                    $liga1=$liga1.$parametros."&sel=Sectores";
                    $texto="Seleccionar";
                    $valcol2="";
                }else{
                    $valcol1="";
                ...
                <tr style="background:<? echo $color; ?>">
                    ...
                    <td class='fcenter'>
                        <? if ($recset["ban"]==0){  ?>
                            <a class='fbutton fwhite fmargin-left fmargin-right' href='<? echo $liga2; ?>'><small>Seleccionar</small></a>
                        <? } ?>    
                    </td>    
                </tr><?                          
                $r++;
...
function flista_niveles_x_sector($nivel_sinpermiso="",$liga1=""){    
    $tot=0; $vl_pontotales=0;
    if ($nivel_sinpermiso!=""){ // arreglo de niveles sin permiso
        $arrnopermiso=explode("|",$nivel_sinpermiso);
        $cad1=""; $cad2=""; foreach ($arrnopermiso as $valor) {if ($valor<10){ $cad1=$cad1.$valor.","; }else{ $cad2=$cad2.$valor."|"; } } 
    ...
    $_sql="select distinct idnivel,nivel,COUNT(distinct cct_zona) AS zonas,COUNT(distinct concat(cct,turno)) AS escuelas FROM ctsev WHERE  cct_sector='".$_SESSION["vg_cct_sector"]."'".$condpermiso." and estatus='A' GROUP BY idnivel";                            
...
function flista_ZONAS_x_almacen_nivel($pmidnivel,$condicion_particular="",$liga1="",$liga2="",$liga3=""){
    ...
    echo "<br><br><br><br>";  
    echo "flista_ZONAS_x_almacen_nivel<br>".$_sql."<br>";
    $res=mysql_query($_sql);    $vl_regs=mysql_num_rows($res);
    if ($vl_regs>0){
...
function flista_ZONAS_x_sector_nivel($pmcct_sector,$pmidnivel,$condicion_particular="",$liga1="",$liga2="",$liga3=""){
    ...
    $totescuelas=0;
    if ($pmidnivel<39){$alma="almarec";}else{$alma="almadis";}
    $_sql="select distinct a.cct_zona,a.zonaescola,b.nombre,b.fer_idniveles,count(a.cct) as escuelas from ctsev a,supervis_c b where a.cct_zona=b.cuenta and a.cct_sector='".$pmcct_sector."' and a.idnivel='".$pmidnivel."' ".$condicion_particular." and a.estatus='A' group by a.cct_zona order by a.cct_zona";
...
function flista_Escuelas_x_zona_nivel($pmcct_zona,$pmidnivel,$condicion_particular="",$liga1="",$liga2="",$liga3=""){
    ...
    $totescuelas=0;
    if ($pmidnivel<39){$alma="almarec";}else{$alma="almadis";}
    $_sql="select distinct cct,turno,nom_ct from ctsev where cct_zona='".$pmcct_zona."' and idnivel='".$pmidnivel."' ".$condicion_particular." and estatus='A' order by cct,turno";
    echo "<br><br><br><br>";  
    ...
    if ($vl_regs>0){
        if ($vl_regs>17){ ?><div class="fix5"> <? } ?>
            <table class="ftable-all"> 
                <tr><th></th><th>cct</th><th>Nombre</th><th>Escuelas</th> <? 
                        if($liga1!=""){ $arr_liga1=explode("|",$liga1); echo "<th>".$arr_liga1[2]."</th>";} // titulo del boton|liga|titulo de la columna 
                        if($liga2!=""){ $arr_liga2=explode("|",$liga2); echo "<th>".$arr_liga2[2]."</th>";} // titulo del boton|liga|titulo de la columna 
                        if($liga3!=""){ $arr_liga3=explode("|",$liga3); echo "<th>".$arr_liga3[2]."</th>";} // titulo del boton|liga|titulo de la columna ?>
                </tr>  <?                $r=0;
