...
<body <? if (!isset($_GET["x"]) && ($_SESSION["vg_tipousuario"]=="Almacen")){ ?> class="fcenter" <?}else{ ?> style="margin-left:auto; margin-right:auto; max-width:1200px; text-align:center; font-size:14px;" <? } ?>>  <?
    ...
    if ((!isset($logeado) && !isset($TxtUsuario))){
        ...
        switch ($_GET["ex"]){
        case 1:  //  ingresa Almacen
            $vl_back="../../_panel.php?papa=1590";
            // ------------------------------------------------------------------- BANNER ENCABEZADO --------------------------------------------------------------- 
            ...
            $_sql="select distinct a.sector,a.cct_sector,b.nombre from ctver_distribucion6 a, sector_c b where a.cct_sector=b.cuenta and a.idalmadis=".$_SESSION["vg_idalmacen"]." and a.idnivel=".$_GET["n"]; 	//echo "<br><br><br><br>".$_sql; 
        ...
        case 2:  //  Ingresa Almacen (de la liga en ex=1) e ingresa Supervisor (de su menu principal)
            ...
            if ($vl_procede==1){ 
                ...
                //---------------------------------------------------------------------------------------------------------------------------------------------- ?>
                            ...
                            <table width="600" cellpadding=5 cellspacing=0 align="center"> <!-- Tabla descriptiva, sin cantidades variables por eso no se usa programa para llenarla -->
                                ...
                                <tr><td>18</td><td style="text-align:left">&emsp;¿Extensión o comunicación?</td><tr>
                ...
                //----------------------------------------------------------------------------------------------------------------------------------------------  ?>
                            <br><br><br>
                            <?  $_sql="select * from ctver_distribucion6 where cct_zona='".$vg_cct_zona."' and idnivel=".$vl_idnivel; 	//echo $_sql; 
                            ...
                            <table cellpadding="7" cellspacing="0" width="100%" border=1><tr><td>Se entrega una Biblioteca con 18 títulos por plantel escolar</td></tr></table>
                            <br><br>
                            ...
                            <tr><th>Sector</th><th>Zona</th><th>Cct-zona</th><th>Nombre</th><th></th></tr><? 
                                while ($r<mysql_num_rows($res)){ 		$recset=mysql_fetch_array($res,MYSQL_ASSOC); $tot=0; 
                                    ...
                                    <tr> 
                                        ...
                                        <td><small><? echo $recset["nombre"]; ?></small></td> 
                                        <td>
                                            <small><a class="fbutton fpadding-tiny fround fbold fpale-green" href="<? echo $autollamada."x&n=".$_GET["n"]."&".$nomarreglo."=".oculta($caddatos,0); ?>">Recibo de Biblioteca SEP</a></small>    
                                        </td> 
                                    </tr> <? 
                                    $r++; 
                                } ?>		 
