...
<body <? if (!isset($_GET["x"]) && ($_SESSION["vg_tipousuario"]=="Almacen")){ ?> class="fcenter" <?}else{ ?> style="margin-left:auto; margin-right:auto; max-width:1200px; text-align:center; font-size:14px;" <? } ?>>  <?
    ...
    if ((!isset($logeado) && !isset($TxtUsuario))){
        ...
        if ($_SESSION["vg_tipousuario"]=="Almacen"){ $liga="https://librosgratuitosveracruz.org/ingresosupervisor/distribucion/recibo_zona_docente_dist21_cal.php?x&n=".$_GET["n"]; }else{ $liga=""; }
        if ($_SESSION["vg_tipousuario"]=="Almacen" && !isset($_GET["x"])){
            $vl_back="../../_panel.php?papa=1880";
                        ...
                        <tr><th>Sector</th><th>Zona</th><th>Cct-zona</th><th>Nombre</th><th></th></tr><? 
                            while ($r<mysql_num_rows($res)){ 		$recset=mysql_fetch_array($res,MYSQL_ASSOC); $tot=0; 
                                ...
                                <tr> 
                                    ...
                                    <td>
                                        <small><a class="fbutton fpadding-tiny fround fbold fpale-green" href="<? echo $autollamada."x&n=".$_GET["n"]."&".$nomarreglo."=".oculta($caddatos,0); ?>">Recibo de Calendario Escolar</a></small>    
                                    </td> 
                                </tr> <? 
                                $r++; 
                            } ?>		 
            ...
            if ($_SESSION["vg_tipousuario"]=="Supervisor" && !isset($_GET["seleccionado"])){
                ...
                // -----------------------------------------------------------------------------------------------------------------------------------------------------
                $liga1="Recibo de Calendario Escolar|".$autollamada."seleccionado&||#F9FFE4|";	//Boton|liga|Columna|color|idnivel|nivelesrestringidos
            ...
            if ($vl_procede==1){ 
                ...
                <div style='margin-left:50px; margin-right:50px;  font-weight:bold'>
                            ...
                            <span style="font-weight:bold; font-size:16px">RECIBO DE ZONA:  "Calendario Escolar 2024-2025" <br>
                ...
                //---------------------------------------------------------------------------------------------------------------------------------------------- ?>
                            ...
                            <table width="700" cellpadding=5 cellspacing=0 align="center"> <!-- Tabla descriptiva, sin cantidades variables por eso no se usa programa para llenarla -->
                                ...
                                <tr><td>CETAM</td><td style="text-align:left">&emsp;Calendario escolar 2024-2025, tipo agenda</td><tr>                         </table>
                            <br><br><p class="SaltoDePagina" style="text-align:left"><small><? echo $liga; ?></small></p>
                            <?
                //----------------------------------------------------------------------------------------------------------------------------------------------  ?>
                            <br><br><br>
                            <? if ($_GET["n"]!=35){ ?>    
                                CCT-ZONA: <? echo $vg_cct_zona; ?>&emsp;&emsp;Nombre: <? echo $rec_zona_alumno["nombre"]; ?>&emsp;&emsp;Asignado: 3
                                <br><br><br>
                            ...
                            <table cellpadding="7" cellspacing="0" width="100%" border=1><tr><td>NOTA : Se entregan 3 ejemplares del Calendario Escolar  por plantel escolar y zona</td></tr></table>
                            <br><br>
