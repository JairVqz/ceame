...
if ((!isset($logeado) && !isset($TxtUsuario))){ 	fmessage("Acceso No Permitido",1500,2,$vl_back);
        ...
        switch($_SESSION["vg_tipousuario"]){
            case "Almacen":
                $vl_back="../ingresoalmacen/pauta_seleccion_niveles_para_sector.php?cct_sector=".$_SESSION["cct_sector"]."&arr_nivel=".$_GET["idnivel"]."|".$_GET["nivel"]."&pmsectores=".$_GET["pmsectores"]."&pmzonas=".$_GET["pmzonas"]."&sel=Zonas";			
                $_SESSION["cct_zona"]=$_GET["cct_zona"];
                $_SESSION["nombre_zona"]=$_GET["nombre"];
                $_SESSION["folio"]=$_GET["folio"];

                $liga="http://www.librosgratuitosveracruz.org/ingresosupervisor/recibo_libros_zona.php?folio=".$_GET["folio"]."&cct_zona=".$_GET["cct_zona"]."&idnivel=".$_GET["idnivel"]."&nivel=".$_SESSION["elnivel"]; 
                break;
            case "Sector":
            ...
            case "Supervisor":
                ...
                if (!isset($_GET["arr_nivel"])){   // INGRESAN TODAS LAS ZONAS AUN LAS DE UN NIVEL !
                    ...
                    if ($_SESSION["vg_tipousuario"]=="Supervisor"){
                        $liga1="Recibo de zona|".$autollamada."sel=reciboalumnos&||#F9FFE4|6";	//Boton|liga|Columna|color|idnivel|nivelesrestringidos
                    }else{
                        $liga1="Recibo de zona|".$autollamada."sel=reciboalumnos&||#F9FFE4|6";	//Boton|liga|Columna|color|idnivel|nivelesrestringidos
                    }
                    ...
                    $liga5="Escuelas|".$autollamada."sel=escuela&||#F9FFE4|6";	//Boton|liga|Columna|color|idnivel|nivelesrestringidos
                    //flista_niveles_x_zona("",$liga1,$liga2,$liga3,$liga4,$liga5);  
                    flista_niveles_x_zona("6",$liga1,$liga5);  
                }else{
                    $vl_back=$autollamada;
                    switch ($_GET["sel"]){
                        case "reciboalumnos":
        ...
        switch($_GET["sel"]){
            case "reciboalumnos": include("recibo_zona_alumnos.php"); break;
            ...
            case "escuela": // aqui ingresa el almacen y sector, pero no el supervisor
                if ($_SESSION["vg_tipousuario"]!="Supervisor"){
                    if ($_SESSION["vg_tipousuario"]=="Almacen"){
                        ...
                        if ($_SESSION["elidnivel"]<40){
                            $condlocal="idalmadis=".$_SESSION["vg_idalmacen"];
                        }else{
                            $condlocal="idalmarbo=".$_SESSION["vg_idalmacen"];
                        }
                        $vl_back="../ingresoalmacen/pauta_seleccion_niveles_para_sector.php?cct_sector=".$_SESSION["cct_sector"]."&idnivel=".$_SESSION["elidnivel"]."&nivel=".$_SESSION["elnivel"]."&arr_nivel=".$_SESSION["elidnivel"]."|".$_SESSION["elnivel"]."&nombre=".$_SESSION["nombre_zona"]."&folio=".$_SESSION["folio"];
                        //if ($_SESSION["vg_idalmacen"]==2){ // condicion solo para xalapa y solo para el año 2024
                            //$liga2="Recibo Escuela (Inglés)|../ingresoplantel/recibo_libros_plantel_sec_ing24.php?vecvesmaest=N&||#F9FFE4";      // titulo del boton|liga|titulo de la columna
                            $liga2="Recibo Escuela (Inglés)|../ingresoplantel/recibo_libros_plantel_sec_ing24.php?||#F9FFE4";      // titulo del boton|liga|titulo de la columna
                        //}
                    }else{
                        $condlocal="";
                    }
