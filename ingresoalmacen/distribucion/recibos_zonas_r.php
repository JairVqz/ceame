...
require_once('../../inc/funciones_libros_r.php'); 
conectalo("almacen");
?>
...
if ((!isset($logeado) && !isset($TxtUsuario))){
    ...
    if (!isset($_GET["arr_nivel"])){ 	
        ...
        // -----------------------------------------------------------------------------------------------------------------------------------------------------
        if (!isset($_SESSION["recibozona"])){$_SESSION["recibozona"]=$_GET["numrecibo"];}
		echo "<br><br>"; flista_niveles($autollamada,"almadis",$_GET["n"]); 
	}else{     
        echo "veremos".$_SESSION["recibozona"];  
        $arrnivel=explode("|",$_GET["arr_nivel"]);
        $elidnivel=$arrnivel[0]; $elnivel=$arrnivel[1]; 

        if ($_SESSION["recibozona"]==1){ $liga="/distribucion/23_febrero_recibo_escuelas_ficherocuadernillo.php?"; }else{ $liga="_reportes_supervisor.php?seleccionado&num_recibo=".$_SESSION["recibozona"]."&"; } // seleccionado es para que entre directo al reporte en _reportes_supervisor.php
        // ------------------------------------------------------------------- BANNER ENCABEZADO --------------------------------------------------------------- 
