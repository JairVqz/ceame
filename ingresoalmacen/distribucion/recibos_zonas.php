...
if ((!isset($logeado) && !isset($TxtUsuario))){
    ...
    if (!isset($_GET["arr_nivel"])){ 	
        ...
        // -----------------------------------------------------------------------------------------------------------------------------------------------------
        if (isset($_GET["recibozona"])){$_SESSION["recibozona"]=$_GET["recibozona"];}
        if (isset($_GET["n"])){$_SESSION["nopermiso"]=$_GET["n"];}else{ $_SESSION["nopermiso"]="";}
		echo "<br><br>"; flista_niveles($autollamada,"almadis",$_GET["n"]); 
	}else{     
        $arrnivel=explode("|",$_GET["arr_nivel"]);
        $elidnivel=$arrnivel[0]; $elnivel=$arrnivel[1]; 
        // FALTA MODIFICAR
        if ($_SESSION["recibozona"]==1){ $liga="distribucion/23_febrero_recibo_escuelas_ficherocuadernillo.php?"; $tit="FICHERO Y CUADERNILLO"; }else{ $liga="_reportes_supervisor.php?seleccionado&num_recibo=".$_SESSION["recibozona"]."&";  $tit="SI TE DROGAS TE DAÑAS";} // seleccionado es para que entre directo al reporte en _reportes_supervisor.php
        // ------------------------------------------------------------------- BANNER ENCABEZADO --------------------------------------------------------------- 
        $TItulo="Almacen ".$logeado;  $subtit="<h3 class='fbold ftext-red fpadding-tiny'>".$pmzonas." zonas de ".$elnivel." para la entrega de ".$tit."</h3>";
        $LGback=$autollamada."n=".$_SESSION["nopermiso"]; $fijoSINO="SI"; 		$stylecolor=""; $lgimg="../../images/logo_SEVGob.png"; $imgancho="800"; $lgagrega=""; $lgbusca="";	fbanner($TItulo,$subtit,$LGback,$fijoSINO,$stylecolor,$lgimg."|".$imgancho,$lgagrega,$lgbusca);	 
        // -----------------------------------------------------------------------------------------------------------------------------------------------------
                            ...
                            <table class="ftable-all"> 
                                <tr><th>Zona</th><th>Clave</th><th>Escuelas</th><th>Municipio</th><th>Localidad</th><th></th></tr><? 
                                    while ($r<mysql_num_rows($res)){ 		$recset=mysql_fetch_array($res,MYSQL_ASSOC);   
                                        // seleccion de colores por nivel :
                                        if ($elidnivel>70){ $color="#FEECEC"; }else{ if ($elidnivel>60){ $color="#F6FECC"; }else{ if ($elidnivel>40){ $color="#E4F4FF"; }else{ $color="#E0FCD6";  }    }     }                ?>
                                        <tr> 
                                            ...
                                            <td align="center"><small><a href="../../ingresosupervisor/<? echo $liga; ?>arr_nivel=<?  echo $arrnivel[0]."|".$arrnivel[1]."&zona=".$recset['cct_zona'];  ?>" class="fbutton ftext-black" style="background:<? echo $color; ?>">Ver escuelas</a></small></td>
                                        </tr> <? 
                                       	$r++; 
                                    } ?>		 
