...
var m1=document.getElementById("error");
m1.innerHTML="";
...
function keynum(e) {
var validchars='1234567890'+String.fromCharCode(46)+String.fromCharCode(36)+String.fromCharCode(16)+String.fromCharCode(39)+String.fromCharCode(35)+String.fromCharCode(37);
 var key='', keychar='';
 key = getKeyCode(e);
//window.alert(key);
 if (key == null) return true;
...
function keynumdec(e) {
var validchars='1234567890'+String.fromCharCode(190)+String.fromCharCode(46)+String.fromCharCode(36)+String.fromCharCode(16)+String.fromCharCode(39)+String.fromCharCode(35)+String.fromCharCode(37);
 var key='', keychar='';
 key = getKeyCode(e);
//window.alert(key);
 if (key == null) return true;
...
/**
 *  ********************************************************************************
 *    Funciones para dos combos
 *  ********************************************************************************
  */
  
  var depende= '../conecta_combos.php';
 /*   -------------- parte variable
 En este Codigo solo modificar los eventos OnChange               */

// constants 
var noValue = '-99';
// default values
var Llave1 = noValue;    var Llave2 = noValue;  var numcombos=noValue;
//selects disabled true/false
var boolEnabled = true;  
// globals
var curOption = new Array(); var isLoaded = new Array();
...
function Combo1_onChange(){
  ...
  if(val == noValue){
	...
	switch(numcbos){  //  generalmente una variable
	case 1:
		 //   -------------- parte variable
  		if(boolEnabled){
	    	document.getElementById('cmdConsultar').disabled="";
    	}		 
		 //    ----------------------------
    case 2:
		 curOption[this.name] = val;
	     emptyList( 'Combo2' );
		 //alert(sacaval(val,1)+'|'+sacaval(val,3)+'+'+sacaval(val,4));
		 jsrsExecute( depende, cbFillDos, 'Lista2',val);
	     window.status = 'Obteniendo datos...';
//		document.getElementById('cmdConsultar').disabled="";	
         break;	
	}
  }  
...
function Combo2_onChange(){
  ...
  if(val == noValue){
    ...
    msg += this.form.Combo1.options[this.form.Combo1.selectedIndex].text + "\n";
...
// ********************************************************************************************
// *********************      Al parecer todo esto ya es fijo     *****************************
// ********************************************************************************************


function cbFillUno ( strMakes ){ 
  window.status = '';
  fillList( 'Combo1',  strMakes ); 
  if(Llave1 != noValue){
    jsrsExecute( depende, cbFillDos, 'Lista2', ''+sacaval(Llave1,1)+'');
  }
}

function cbFillDos( strModels ){ 
  // callback for dependent listbox
  window.status = '';
  fillList( 'Combo2', strModels ); 
}

function fillList( NameCombo, strOptions ){
  // fill any list with options
  emptyList( NameCombo );
  
  // always insert selection prompt
  var lst = document.forms['frmactual'][NameCombo];
  lst.disabled = true;
  ...
  // init to no value
  selectOption( NameCombo, ID );
  isLoaded[NameCombo] = true;
  lst.disabled = !boolEnabled;
  lst.onchange = eval( NameCombo + "_onChange" );
  // eval( "document.forms['frmactual']['" + listName + "'].onchange=" + listName + "_onChange;" );
}

// ********************************************************************************************
function preselect(numcombos,p_key1,p_key2,_sql1_java,selectable){
  boolEnabled = selectable;
  Llave1 = p_key1;
  Llave2 = p_key2;
  initLists(numcombos,_sql1_java);
}

function initLists(p_numcbos,pm_sql1_java){
  // initialize lists
  numcbos=p_numcbos;
  switch(numcbos){  //  generalmente una variable
    case 1:
  ...
  jsrsExecute( depende, cbFillUno, 'Lista1',pm_sql1_java);
}


function emptyList( listName ){
  var lst = document.forms['frmactual'][listName];
...
function jsrsDebugInfo(){
  ...
  /**
 *  ********************************************************************************
 *    FIN DE Funciones para dos combos
 *  ********************************************************************************
  */
...
Calendar._TT["TODAY"] = "Seleccione Hoy";
